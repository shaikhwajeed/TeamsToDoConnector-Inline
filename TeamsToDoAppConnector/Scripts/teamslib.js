...
(function (microsoftTeams) {
   ...
   */
    ...
    (function (authentication) {
        ...
        * Hide from docs.
        ...
        function startAuthenticationWindowMonitor() {
            ...
            handlers["navigateCrossDomain"] = function () {
                return false;
            };
        }
        /**
    ...
    })(microsoftTeams.authentication || (microsoftTeams.authentication = {}));
    function ensureInitialized() {
        ...
        if (frameContext && expectedFrameContexts.length > 0) {
            var found = false;
            for (var j = 0; j < expectedFrameContexts.length; j++) {
                if (expectedFrameContexts[j] === frameContext) {
                    found = true;
                    break;
                }
            }
    ...
    function handleParentMessage(evt) {
        if ("id" in evt.data) {
            ...
            var callback = callbacks[messageData.id];
            if (callback) {
                callback.apply(null, messageData.args);
                // Remove the callback to ensure that the callback is called only once and to free up memory.
                delete callbacks[messageData.id];
            }
        }
